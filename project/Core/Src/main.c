...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of myQueue */
  myQueueHandle = osMessageQueueNew (16, sizeof(Data), &myQueue_attributes);
...
void StartProducer(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  data.pressure = baro_read_press();

	  osMessageQueuePut(myQueueHandle, &data, 0U, osWaitForever);
...
void StartConsumer(void *argument)
{
  ...
  {
	  Data data;
	  status = osMessageQueueGet(myQueueHandle, &data, NULL, osWaitForever);

	  if (status == osOK) {
		  snprintf(text, countof(text), "/* %ld.%02ld, %ld.%02ld */\n", (data.temp)/100, (data.temp)%100, (data.pressure)/100, (data.pressure)%100);
		  HAL_UART_Transmit(&huart1, (uint8_t*)text, strlen(text), HAL_MAX_DELAY);
	  }
	  else {
		  snprintf(text, countof(text), "Consumer ERROR\n");
		  HAL_UART_Transmit(&huart1, (uint8_t*)text, strlen(text), HAL_MAX_DELAY);
	  }
