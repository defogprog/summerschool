...
struct {
	...
	int16_t  dig_P8;
	int16_t  dig_P9;
} /* __attribute__((packed)) */ static BARO_trimming;
...
baro_stat_t baro_init(void) {
	...
	status = HAL_I2C_Mem_Read(&hi2c1, BARO_I2C_ADDR, BARO_REG_CALIB00, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&BARO_trimming, sizeof(BARO_trimming), 1000);
	if (status != HAL_OK) {
		if (status == HAL_TIMEOUT)
			return BARO_ERR_TIMEOUT;
		else
...
int32_t baro_read_temp(void) {
	...
	var1 = ((((adc_T >> 3) - ((int32_t)BARO_trimming.dig_T1 << 1))) *
		   ((int32_t)BARO_trimming.dig_T2)) >> 11;

	var2 = (((((adc_T >> 4) - ((int32_t)BARO_trimming.dig_T1)) *
			  ((adc_T >> 4) - ((int32_t)BARO_trimming.dig_T1))) >> 12) *
		      ((int32_t)BARO_trimming.dig_T3)) >> 14;
...
uint32_t baro_read_press(void) {
	...
	var2 = var1 * var1 * (int64_t)BARO_trimming.dig_P6;
	var2 = var2 + ((var1 * (int64_t)BARO_trimming.dig_P5) << 17);
	var2 = var2 + (((int64_t)BARO_trimming.dig_P4) << 35);
	var1 = ((var1 * var1 * (int64_t)BARO_trimming.dig_P3) >> 8) +
		   ((var1 * (int64_t)BARO_trimming.dig_P2) << 12);
	var1 = (((((int64_t)1) << 47) + var1)) * ((int64_t)BARO_trimming.dig_P1) >> 33;
	...
	var1 = (((int64_t)BARO_trimming.dig_P9) * (p >> 13) * (p >> 13)) >> 25;
	var2 = (((int64_t)BARO_trimming.dig_P8) * p) >> 19;

	p = ((p + var1 + var2) >> 8) + (((int64_t)BARO_trimming.dig_P7) << 4);
	return p / 256;
